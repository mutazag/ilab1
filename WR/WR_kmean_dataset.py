...
########################################################################################################################
########################################################################################################################
#Create randomized k-means training and test sets for the side-by-side
########################################################################################################################
########################################################################################################################
########################################################################################################################
"""
...
from sklearn.cluster import KMeans
from pandas import DataFrame
train_df = pd.read_csv("LASSO_SideBySide/side_by_side_trainingset.csv",header=None)
numColumns = train_df.shape[1]
x_reduced = train_df.iloc[:,0:numColumns-1]
y_reduced = train_df.iloc[:,numColumns-1]

kmeans = KMeans(n_clusters=4, random_state=None).fit(x_reduced,y_reduced)
for i in range(x_reduced.shape[0]):
    print(y_reduced.iloc[i],kmeans.labels_[i])
    

numDescriptors=int((train_df.shape[1]-1)/2)
numDescriptors*=2
numDescriptors+=1
klabels=kmeans.labels_
train_df['kmeans']=0
for i in range(0,len(kmeans.labels_)):
    train_df['kmeans'][2*i]=klabels[i]
    train_df['kmeans'][2*i+1]=klabels[i]
...
testportion=cluster0.iloc[0:int(cluster0Portion),0:numDescriptors]
trainingportion=cluster0.iloc[int(cluster0Portion):cluster0.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster0.shape,kmeansTestSet_df.shape,int(cluster0Portion))

testportion=cluster1.iloc[0:int(cluster1Portion),0:numDescriptors]
trainingportion=cluster1.iloc[int(cluster1Portion):cluster1.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster1.shape,kmeansTestSet_df.shape,int(cluster1Portion))

testportion=cluster2.iloc[0:int(cluster2Portion),0:numDescriptors]
trainingportion=cluster2.iloc[int(cluster2Portion):cluster2.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster2.shape,kmeansTestSet_df.shape,int(cluster2Portion))

testportion=cluster3.iloc[0:int(cluster3Portion),0:numDescriptors]
trainingportion=cluster3.iloc[int(cluster3Portion):cluster3.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster3.shape,kmeansTestSet_df.shape,int(cluster3Portion))

testportion=cluster4.iloc[0:int(cluster4Portion),0:numDescriptors]
trainingportion=cluster4.iloc[int(cluster4Portion):cluster4.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster4.shape,kmeansTestSet_df.shape,int(cluster4Portion))
#
testportion=cluster5.iloc[0:int(cluster5Portion),0:numDescriptors]
trainingportion=cluster5.iloc[int(cluster5Portion):cluster5.shape[0],0:numDescriptors]
kmeansTestSet_df=kmeansTestSet_df.append(testportion)
kmeansTrainingSet_df=kmeansTrainingSet_df.append(trainingportion)
print(cluster5.shape,kmeansTestSet_df.shape,int(cluster5Portion))

kmeansTrainingSet_df.to_csv("LASSO_SideBySide/kmeans_randomized_trainingset_"+str(count)+"_indexed.csv",header=False)
kmeansTestSet_df.to_csv("LASSO_SideBySide/kmeans_randomized_testset_"+str(count)+"_indexed.csv",header=False)

kmeansTrainingSet_df.to_csv("LASSO_SideBySide/kmeans_randomized_trainingset_"+str(count)+"_unindexed.csv",header=False,index=False)
kmeansTestSet_df.to_csv("LASSO_SideBySide/kmeans_randomized_testset_"+str(count)+"_unindexed.csv",header=False,index=False)
...
########################################################################################################################
########################################################################################################################
#Create randomized k-means training and test sets for the side-by-side, multiple records, ensuring no common
#data between training and test sets
########################################################################################################################
########################################################################################################################
########################################################################################################################
...
count=0
train_df = pd.read_csv("LASSO_SideBySideMultiple_Corrected/side_by_side_trainingset.csv",header=None)
numColumns = train_df.shape[1]
x_reduced = train_df.iloc[:,0:numColumns-1]
y_reduced = train_df.iloc[:,numColumns-1]
from pandas import DataFrame
...
cluster5=train_df.loc[train_df.kmeans==5]
#Randomize
#DO NOT MIX THE COPIED RECORDS, OTHERWISE THERE WILL BE OVERLAP BETWEEN TRAINING AND TEST SETS!!
cluster0=cluster0.sample(frac=1)
cluster1=cluster1.sample(frac=1)
cluster2=cluster2.sample(frac=1)
cluster3=cluster3.sample(frac=1)
...
kmeansTrainingSet_df=kmeansTrainingSet_df.append(kmeansTrainingSet_df)
kmeansTrainingSet_df=kmeansTrainingSet_df.sample(frac=1)

kmeansTrainingSet_df.to_csv("LASSO_SideBySideMultiple_Corrected/kmeans_randomized_trainingset_"+str(count)+"_indexed.csv",header=False)
kmeansTestSet_df.to_csv("LASSO_SideBySideMultiple_Corrected/kmeans_randomized_testset_"+str(count)+"_indexed.csv",header=False)

kmeansTrainingSet_df.to_csv("LASSO_SideBySideMultiple_Corrected/kmeans_randomized_trainingset_"+str(count)+"_unindexed.csv",header=False,index=False)
kmeansTestSet_df.to_csv("LASSO_SideBySideMultiple_Corrected/kmeans_randomized_testset_"+str(count)+"_unindexed.csv",header=False,index=False)
"""